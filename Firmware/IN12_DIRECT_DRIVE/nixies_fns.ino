void setDigit(uint8_t digit, uint8_t value) {
  ...
  for (int i = 0; i < sizeof(pinsFix[digit]); i++) {
    shift.set(pinsFix[digit][i], digitPins[digit][value][i]);
  }
  if (digit == 0) {
    analogWrite(5, bri_vals[bri]);
  } else {
    analogWrite(4, bri_vals[bri]);
  }
}
...
void blankDigit(uint8_t digit) {
  //setDigit(digit, 10);
  if (digit == 0) {
    digitalWrite(5, 0); // first digit allows anode blanking
  } else {
    setDigit(digit, 0); // Set to 0 instead for compatibility with MH74141 as it doesn't allow cathode blankign like the russian IC.
  }
}
